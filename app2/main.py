...
def messages():
    if request.method == 'POST':
        ...
        if len(message) < 1:
            ...
            message_encrypted = fernet.encrypt(message.encode()).decode()

            # Se envia el mensaje
            try:
                response = requests.post('http://127.0.0.1:5001/enviar', json={'usuario':store['usuario'],'destinatario':store['destinatario'],'message': message_encrypted, 'timestamp': timestamp})
                if response.status_code == 200:
